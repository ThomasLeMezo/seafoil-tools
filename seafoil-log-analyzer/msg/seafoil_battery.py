...
class SeafoilBattery(SeafoilData):
    def __init__(self, bag_path="", topic_name="", start_date=datetime.datetime(2019, 1, 1)):
        ...
        self.average_current = np.empty([self.nb_elements], dtype='uint16')
        self.standby_current = np.empty([self.nb_elements], dtype='uint16')
        self.max_load_current = np.empty([self.nb_elements], dtype='uint16')
        self.average_power = np.empty([self.nb_elements], dtype='uint16')
